...
from base_services import ChatService, RetrieveService
...
class SyftLLMRouter:
    """Syft LLM Router that orchestrates chat and retrieve services."""
    ...
    def retrieve_documents(
        ...
        if not self.retrieve_service:
            raise NotImplementedError("Retrieve functionality is not enabled")
        return self.retrieve_service.retrieve_documents(query, options)
'''
...
version = "0.1.0"
description = "A Syft LLM Router with configurable chat and retrieve services"
authors = [
    {{name = "Generated by Syft LLM Router Generator"}}
]
readme = "README.md"
requires-python = ">=3.9"
dependencies = [
...
"""
        ...
        # Also generate a requirements.txt for backward compatibility
        # with the base dependencies only
        base_requirements = [
            "fastapi>=0.104.0",
            "uvicorn[standard]>=0.24.0",
            "pydantic>=2.0.0",
    ...
    def _generate_env_example(self, output_path: Path, config: ProjectConfig) -> None:
        ...
        env_vars = [
            ...
            f"ENABLE_RETRIEVE={str(config.enable_retrieve).lower()}",
            "",
        ]
        ...
        if config.router_type == "default":
            ...
            env_vars.extend(
                [
                    ...
                    "# CUSTOM_RETRIEVE_API_KEY=your_api_key",
                    "# CUSTOM_RETRIEVE_BASE_URL=your_base_url",
                ]
            )
    ...
    def _generate_readme(self, output_path: Path, config: ProjectConfig) -> None:
        ...
        if config.router_type == "default":
            ...
            if config.enable_retrieve:
                enabled_services.append("Retrieve (Local RAG)")

            services_text = ", ".join(enabled_services) if enabled_services else "None"

            readme_content = f"""# {config.project_name}

A Syft LLM Router with default implementations for chat (Ollama) and retrieve (Local RAG).

## Features

- ✅ **Chat Service**: Ollama integration for local LLM chat
- ✅ **Retrieve Service**: Local ChromaDB for document retrieval
...
- `POST /retrieve` - Document retrieval (if enabled)
...
- **Retrieve Service**: `pip install -e .[retrieve]`
...
A custom Syft LLM Router template. Implement your own chat and retrieve services.

## Features

- 🔧 **Custom Chat Service**: Implement your own chat logic
- 🔧 **Custom Retrieve Service**: Implement your own RAG logic
- ✅ **FastAPI Server**: RESTful API endpoints
- ✅ **Health Monitoring**: Built-in health checks
- ✅ **Validation**: Automated testing and validation
...
2. **Edit `retrieve_service.py`**: Implement your retrieve service
...
echo "💡 To install additional services later, run: pip install -e .[chat,retrieve]"
deactivate
"""
        else:  # custom
            setup_script_content = f"""#!/bin/bash
...
"""
        ...
        if config.enable_retrieve:
            setup_commands.append(
                """
# Local RAG setup (for retrieve service)
echo "🔍 Setting up local RAG components..."

# Create ChromaDB directory if it doesn't exist
mkdir -p ./chroma_db

# Download sentence transformer model if not cached
python -c "
...
"""
    ...
    def _generate_validation_script(
        ...
        if config.enable_retrieve:
            tests_to_run.append("test_retrieve_endpoint")
...
def test_retrieve_endpoint():
    \"\"\"Test retrieve endpoint if enabled.\"\"\"
    try:
        payload = {{
            "query": "test query",
            "options": {{"limit": 5}}
        }}
        response = requests.post("http://localhost:8000/retrieve", json=payload, timeout=10)
        if response.status_code == 200:
            print("✅ Retrieve endpoint test passed")
            return True
        else:
            print(f"❌ Retrieve endpoint test failed: {{response.status_code}}")
            return False
    except requests.exceptions.RequestException as e:
        print(f"❌ Retrieve endpoint test failed: {{e}}")
        return False

def main():
    \"\"\"Run validation tests.\"\"\"
    print(f"🧪 Testing {{config.project_name}} router...")
    print(f"🔧 Enabled services: Chat={{config.enable_chat}}, Retrieve={{config.enable_retrieve}}")
    ...
    test_functions = {{
        ...
        "test_retrieve_endpoint": test_retrieve_endpoint
    }}
...
def main():
    ...
    parser.add_argument(
        "--disable-retrieve",
        action="store_true",
        help="Disable retrieve service (default: enabled)",
    )
    ...
    try:
        ...
        enable_retrieve = not args.disable_retrieve
        ...
        print(f"🔍 Retrieve: {'✅' if config.enable_retrieve else '❌'}")
